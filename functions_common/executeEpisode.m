function [examples,PI,Reward,hist_defaultpolicy_reward,hist_max_reward,ifcollision,hist_success,hist_r, poseStop, updateCount, success_final_flag]= executeEpisode(s,ku, xs, ys, angle_real_start)%初始位置/库位
...
%         p_s_a_15 = VA_NN_201126(x,y,theta,vorder,Action_real(end));
        ...
        [x_next, y_next, theta_next,~] = kinematic_ds(x,y,theta,vorder_next,action_next,0);
...
%         disp('updated')
    ...
    theta=s0(3);
    ITER=ITER+1;
    [x_new,y_new,theta_new,~]=kinematic_ds(x,y,theta,vorder,action,0);%
